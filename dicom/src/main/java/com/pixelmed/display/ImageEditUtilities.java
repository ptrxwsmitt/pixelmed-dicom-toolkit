...
public class ImageEditUtilities {
	private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/display/ImageEditUtilities.java,v 1.42 2022/01/21 19:51:20 dclunie Exp $";
...
//	}
	 ...
	 * @param	shapes	a {@link Vector java.util.Vector} of {@link Shape java.awt.Shape}, specifed in image-relative coordinates
	 * @return			the temporary File used to store the JPEG frames, if any, already set to deleteOnExit()
	 ...
	 */
	static public File blackoutJPEGBlocks(AttributeList list,Vector shapes) throws DicomException, IOException, Exception {
			...
			if (shapes != null && shapes.size() > 0) {
				if (aPixelData instanceof OtherByteAttributeMultipleCompressedFrames) {
					...
					for (int f=0; f<frames.length; ++f) {
						...
						com.pixelmed.codec.jpeg.Parse.parse(fbis,fbos,shapes);		// does the redaction
		...
		return temporaryFrameDataOutputFile;
	}
	...
	static public void blackoutJPEGBlocks(File srcFile,File dstFile,Vector shapes) throws DicomException, IOException, Exception {
		...
		File temporaryFrameDataOutputFile = blackoutJPEGBlocks(list,shapes);
	 ...
	 * See, for example, {@link AttributeList#correctDecompressedImagePixelModule() AttributeList.correctDecompressedImagePixelModule()}.</p>
	 *

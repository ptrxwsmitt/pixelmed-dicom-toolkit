 ...
 * referenced images.</p>
 *
 * <p>Essentially an example of overriding {@link StructuredReportBrowser#doSomethingWithSelectedSOPInstances(Vector instances) doSomethingWithSelectedSOPInstances()}
 * in {@link StructuredReportBrowser StructuredReportBrowser}, and passing
 * pre-defined graphic shapes and text to {@link SingleImagePanel SingleImagePanel}.</p>
 *
 * @see StructuredReportBrowser
 * @see DisplayStructuredReportBrowser
...
public class DicomBrowser extends ApplicationFrame {
	 ...
	 * <p>Construct a browser.</p>
	 *
	 * @param	closeOperation	argument to {@link JFrame#setDefaultCloseOperation(int) setDefaultCloseOperation()}
	 */
...
//System.err.println(fileName);
			try {
				...
				try {
					...
					di=new DicomInputStream(new FileInputStream(fileName.toLowerCase(Locale.US)));
				}
	...
	public static void main(String arg[]) {
		try {
			...
			if (arg.length == 1 || arg.length == 3) {
				...
				EventQueue.invokeAndWait(fileChooserThread);

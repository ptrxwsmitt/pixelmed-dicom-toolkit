...
class AssociationInitiator extends Association {
	...
	protected AssociationInitiator(String hostname,int port,String calledAETitle,
		...
		if (slf4jlogger.isTraceEnabled()) slf4jlogger.trace("Association[{}]: Us:\n{}",associationNumber,arq.toString());
												// State 1 - Idle
		...
		try {
			...
			if (slf4jlogger.isTraceEnabled()) slf4jlogger.trace("took {} mS for new Socket to return",(System.currentTimeMillis() - startOfConnection));
			...
			if (slf4jlogger.isTraceEnabled()) slf4jlogger.trace("took {} mS from requesting new Socket to get input stream",(System.currentTimeMillis() - startOfConnection));
			//in = new MonitoredInputStream(socket.getInputStream(),inputTransferMonitoringContext);
			out = socket.getOutputStream();
			if (slf4jlogger.isTraceEnabled()) slf4jlogger.trace("took {} mS from requesting new Socket to get output stream",(System.currentTimeMillis() - startOfConnection));
			...
			out.flush();
			if (slf4jlogger.isTraceEnabled()) slf4jlogger.trace("took {} mS from requesting new Socket to back from writing A-ASSOCIATE-RQ PDU",(System.currentTimeMillis() - startOfConnection));
			...
			if (slf4jlogger.isTraceEnabled()) slf4jlogger.trace("took {} mS from requesting new Socket to back from reading A-ASSOCIATE-AC or -RJ PDU",(System.currentTimeMillis() - startOfConnection));
			...
			if (pduType == 0x02) {							//           - A-ASSOCIATE-AC PDU
				...
				if (slf4jlogger.isTraceEnabled()) slf4jlogger.trace("Association[{}]: Accepted presentation contexts:\n{}",associationNumber,this.presentationContexts.toString());
		...
		if (slf4jlogger.isTraceEnabled()) slf4jlogger.trace("took {} mS to establish Association",(System.currentTimeMillis() - startOfAssociationEstablishment));
	}

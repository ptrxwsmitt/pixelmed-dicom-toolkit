...
public class NIfTI1ToDicom {
	private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/convert/NIfTI1ToDicom.java,v 1.38 2022/01/21 19:51:13 dclunie Exp $";
	...
	public static AttributeList generateGeometryFunctionalGroupsFromNIfTI1Header(NIfTI1Header nifti,AttributeList list,int numberOfFrames) throws DicomException {
		...
		float qfac = 0;
		{
			...
			if ((preferSForm || nifti.qform == NIfTI1Header.CoordinateTransform.UNKNOWN) && nifti.sform != NIfTI1Header.CoordinateTransform.UNKNOWN) {
				...
				aImageOrientationPatient.addValue(   nifti.srow_z[0] / nifti.pixdim[1]);
				...
				aImageOrientationPatient.addValue(   nifti.srow_z[1] / nifti.pixdim[2]);
			}
		...
		
		{
			...
			// and for > 4 dimensions, need to use the 4th temporal dimension to increment the Temporal Position Index after every dim[3] frames ...
			int numberOfTemporalPositionIndices = nifti.dim[0] >= 4 ? (nifti.dim[4] > 0 ? nifti.dim[4] : 1) : 1;
			...
			if ((preferSForm || nifti.qform == NIfTI1Header.CoordinateTransform.UNKNOWN) && nifti.sform != NIfTI1Header.CoordinateTransform.UNKNOWN) {
				...
				for (int f=0; f<numberOfFrames; ++f) {
					...
					a.addValue(nx * qfac * nifti.pixdim[3] * frameOffsetInStack - nifti.qoffset_x);		// NIfTI is RAS+, DICOM is LPS+
					a.addValue(ny * qfac * nifti.pixdim[3] * frameOffsetInStack - nifti.qoffset_y);		// NIfTI is RAS+, DICOM is LPS+
			...
			{
				int f=0;
				while (f<numberOfFrames) {	// Why this outer loop ? There may be more dimensions but we don't know what they are
					for (int temporalPositionIndex=1; temporalPositionIndex<=numberOfTemporalPositionIndices; ++temporalPositionIndex) {
						for (int inStackPositionNumber=1; inStackPositionNumber<=framesInStack; ++inStackPositionNumber) {
							{
								SequenceAttribute aFrameContentSequence = new SequenceAttribute(TagFromName.FrameContentSequence);
								SequenceAttribute.getAttributeListFromSelectedItemWithinSequence(aPerFrameFunctionalGroupsSequence,f).put(aFrameContentSequence);
								
								AttributeList itemList = new AttributeList();
								aFrameContentSequence.addItem(itemList);
								
								{ Attribute a = new ShortStringAttribute(TagFromName.StackID); a.addValue(1); itemList.put(a); }
								{ Attribute a = new UnsignedLongAttribute(TagFromName.InStackPositionNumber); a.addValue(inStackPositionNumber); itemList.put(a); }
								{ Attribute a = new UnsignedLongAttribute(TagFromName.TemporalPositionIndex); a.addValue(temporalPositionIndex); itemList.put(a); }
								// order of DimensionIndexValues must match the order in DimensionIndexSequence of Dimensions Module
								{ Attribute a = new UnsignedLongAttribute(TagFromName.DimensionIndexValues); a.addValue(1); a.addValue(inStackPositionNumber); a.addValue(temporalPositionIndex); itemList.put(a); }
							}
							++f;	// in the innermost loop
						}
					}
				}
			}
	...
	public static AttributeList generateDimensions(AttributeList list) throws DicomException {
		...
		// specify order of dimension indices as reverse of how they are encoded in NIfTI and in PixelData, by StackID first, then by InStackPositionNumber, then by TemporalPositionIndex
		{
			...
			}
			{
				AttributeList itemList = new AttributeList();
				saDimensionIndexSequence.addItem(itemList);
				{ AttributeTagAttribute a = new AttributeTagAttribute(TagFromName.DimensionIndexPointer); a.addValue(TagFromName.TemporalPositionIndex); itemList.put(a); }
				{ AttributeTagAttribute a = new AttributeTagAttribute(TagFromName.FunctionalGroupPointer); a.addValue(TagFromName.FrameContentSequence); itemList.put(a); }
				{ Attribute a = new UniqueIdentifierAttribute(TagFromName.DimensionOrganizationUID); a.addValue(dimensionOrganizationUID); itemList.put(a); }
				{ Attribute a = new LongStringAttribute(TagFromName.DimensionDescriptionLabel); a.addValue("TemporalPositionIndex"); itemList.put(a); }
			}
		}
		return list;
	}

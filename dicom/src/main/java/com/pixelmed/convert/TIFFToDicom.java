...
public class TIFFToDicom {
    ...
    private final UIDGenerator uidGen = new UIDGenerator();
    ...
    private class UIDMap {
        ...
        private class UIDMapCSVReadProc {
            ...
            public void procRow(String... values) {
        ...
        UIDMap(String uidFileName) {
            ...
            final var lineReader = new UIDMapCSVReadProc();
            try (
                Reader reader = Files.newBufferedReader(Path.of(uidFileName));
                CSVReader csvReader = new CSVReader(reader)
            ) {
                String[] line;
                while ((line = csvReader.readNext()) != null) {
                    lineReader.procRow(line);
                }
            } catch (CsvValidationException | IOException e) {
                slf4jlogger.error("Error reading UID file {}", uidFileName, e);
            }
        }
        ...
        void replaceUIDs(AttributeList list, String inputFileName, int dirNum) throws DicomException {
            {
                ...
                if (uid != null && uid.length() > 0) {
                    ...
                    Attribute a;
                    a = new UniqueIdentifierAttribute(TagFromName.StudyInstanceUID);
                    a.addValue(uid);
                    list.put(a);
                }
            }
            {
                ...
                if (uid != null && uid.length() > 0) {
                    ...
                    Attribute a;
                    a = new UniqueIdentifierAttribute(TagFromName.SeriesInstanceUID);
                    a.addValue(uid);
                    list.put(a);
                }
            }
            {
                ...
                if (uid != null && uid.length() > 0) {
                    ...
                    Attribute a;
                    a = new UniqueIdentifierAttribute(TagFromName.SOPInstanceUID);
                    a.addValue(uid);
                    list.put(a);
                }
            }
            ...
            }
            {
                ...
                if (uid != null && uid.length() > 0) {
                    ...
                    Attribute a;
                    a = new UniqueIdentifierAttribute(TagFromName.PyramidUID);
                    a.addValue(uid);
                    list.put(a);
                }
            }
            {
                ...
                if (uid != null && uid.length() > 0) {
                    ...
                    Attribute a;
                    a = new UniqueIdentifierAttribute(TagFromName.AcquisitionUID);
                    a.addValue(uid);
                    list.put(a);
                }
            }
                ...
                if (uid != null && uid.length() > 0) {
                    ...
                    if (aSpecimenDescriptionSequence != null) {
                        ...
                        while (sitems.hasNext()) {
                            ...
                            {
                                Attribute a = new UniqueIdentifierAttribute(TagFromName.SpecimenUID);
                                a.addValue(uid);
                                itemList.put(a);
                            }
                        }
                    ...
                    slf4jlogger.debug("Using supplied DimensionOrganizationUID {}", uid);
                    {
                        ...
                        if (aDimensionOrganizationSequence != null) {
                            ...
                            while (sitems.hasNext()) {
                                AttributeList itemList = sitems.next().getAttributeList();
                                {
                                    Attribute a = new UniqueIdentifierAttribute(TagFromName.DimensionOrganizationUID);
                                    a.addValue(uid);
                                    itemList.put(a);
                                }
                            }
    ...
    private void addTotalPixelMatrixOriginSequence(AttributeList list, double xOffsetInSlideCoordinateSystem, double yOffsetInSlideCoordinateSystem) throws DicomException {
        ...
        list.put(aTotalPixelMatrixOriginSequence);
        {
            ...
            {
                Attribute a = new DecimalStringAttribute(DicomDictionary.StandardDictionary.getTagFromName("XOffsetInSlideCoordinateSystem"));
                a.addValue(xOffsetInSlideCoordinateSystem);
                itemList.put(a);
            }
            {
                Attribute a = new DecimalStringAttribute(DicomDictionary.StandardDictionary.getTagFromName("YOffsetInSlideCoordinateSystem"));
                a.addValue(yOffsetInSlideCoordinateSystem);
                itemList.put(a);
            }
        }
    }
    ...
    private long generateDICOMPixelDataMultiFrameImageFromTIFFFile(TIFFFile inputFile, AttributeList list,
        ...
        if (compression == 0 || compression == 1    // absent or specified as uncompressed
            ...
            if (recompressAsFormat == null || recompressAsFormat.length() == 0) {
                    ...
                    if (mergeSamplesPerPixelTiles) {    // (001351)
                        ...
                        for (int destinationTileNumber = 0; destinationTileNumber < numberOfDestinationTiles; ++destinationTileNumber) {
                            ...
                            for (int c = 0; c < samplesPerPixel; ++c) {
                                ...
                                if (slf4jlogger.isTraceEnabled())
                                    slf4jlogger.trace("generateDICOMPixelDataMultiFrameImageFromTIFFFile(): decompressed LZW tile length = {}", sourceValues.length);
                            ...
                            if (slf4jlogger.isTraceEnabled())
                                slf4jlogger.trace("Tile {} created uncompressed temporary file {}", destinationTileNumber, tmpFile.toString());
                            ...
                            BufferedOutputStream o = new BufferedOutputStream(new FileOutputStream(tmpFile));
                            if (slf4jlogger.isTraceEnabled())
                                slf4jlogger.trace("Tile {} writing {} bytes to temporary file {}", destinationTileNumber, destinationValues.length, tmpFile.toString());
                            o.write(destinationValues);
                            o.flush();
                            o.close();
                            if (slf4jlogger.isTraceEnabled())
                                slf4jlogger.trace("Tile {} after write, temporary file length = {}", destinationTileNumber, tmpFile.length());
                        }
                    } else {
                        files = new File[numberOfDestinationTiles];
                        for (int tileNumber = 0; tileNumber < numberOfDestinationTiles; ++tileNumber) {
                            ...
                            if (slf4jlogger.isTraceEnabled())
                                slf4jlogger.trace("generateDICOMPixelDataMultiFrameImageFromTIFFFile(): decompressed LZW tile length = {}", values.length);
                            // apply horizontal differencing as per ij/io/ImageReader.java and TIFF6 page 64
                            if (predictor == 2) {    // horizontalDifferencing
                            ...
                            tmpFile.deleteOnExit();
                            if (slf4jlogger.isTraceEnabled())
                                slf4jlogger.trace("Tile {} created uncompressed temporary file {}", tileNumber, tmpFile.toString());
                            if (filesToDeleteAfterWritingDicomFile == null) {
                                filesToDeleteAfterWritingDicomFile = new ArrayList<File>();
                            } else {
                                filesToDeleteAfterWritingDicomFile.add(tmpFile);
                            ...
                            BufferedOutputStream o = new BufferedOutputStream(new FileOutputStream(tmpFile));
                            if (slf4jlogger.isTraceEnabled())
                                slf4jlogger.trace("Tile {} writing {} bytes to temporary file {}", tileNumber, values.length, tmpFile.toString());
                            o.write(values);
                            o.flush();
                            o.close();
                            if (slf4jlogger.isTraceEnabled())
                                slf4jlogger.trace("Tile {} after write, temporary file length = {}", tileNumber, tmpFile.length());
                        }
                    }
                } else {
                ...
                if (bitsPerSample == 8) {
                    ...
                    for (int tileNumber = 0; tileNumber < numberOfDestinationTiles; ++tileNumber) {
                        ...
                        tmpFile.deleteOnExit();
                        if (slf4jlogger.isTraceEnabled())
                            slf4jlogger.trace("Tile {} created compressed temporary file {}", tileNumber, tmpFile.toString());
            ...
            for (int tileNumber = 0; tileNumber < numberOfDestinationTiles; ++tileNumber) {
                ...
                o.close();
                if (slf4jlogger.isTraceEnabled())
                    slf4jlogger.trace("Tile {} wrote {} bytes to {}", tileNumber, values.length, tmpFile.toString());
    ...
    private static AttributeList generateDICOMPixelDataModuleAttributes(AttributeList list,
        ...
        switch ((int) photometric) {
            ...
            case 3:
                ...
                break;
            case 4:
                photometricInterpretation = "TRANSPARENCY";
                break;        // not standard DICOM
            case 5:
                photometricInterpretation = "CMYK";
                break;                // retired in DICOM
            case 6:
                photometricInterpretation = (recompressAsFormat != null && recompressAsFormat.equals("jpeg2000")) ? (recompressLossy ? "YBR_ICT" : "YBR_RCT") : "YBR_FULL_422";
                break;
            case 8:
...

        {
            ...
            a.addValue((int) samplesPerPixel);
            list.put(a);
        }
        ...
        if (samplesPerPixel > 1) {
            ...
            a.addValue((int) planarConfig - 1);
            list.put(a);    // TIFF is 1 or 2 but sometimes absent (0), DICOM is 0 or 1
        }
    ...
    private byte[] addICCProfileToOpticalPathSequence(AttributeList list, byte[] iccProfile) throws DicomException {
        ...
        if (opticalPathSequenceItemList != null) {
            if (iccProfile == null || iccProfile.length == 0) {
                ...
                {
                    Attribute a = new CodeStringAttribute(DicomDictionary.StandardDictionary.getTagFromName("ColorSpace"));
                    a.addValue("SRGB");
                    opticalPathSequenceItemList.put(a);
                }
            } else {
        ...
        if (objectiveLensPower != 0) {
            ...
            if (opticalPathSequenceItemList != null) {
                ...
                if (existingObjectiveLensPower == 0) {
                    {
                        Attribute a = new DecimalStringAttribute(DicomDictionary.StandardDictionary.getTagFromName("ObjectiveLensPower"));
                        a.addValue(objectiveLensPower);
                        opticalPathSequenceItemList.put(a);
                    }
        ...
        if (objectiveLensNumericalAperture != 0) {
            ...
            if (opticalPathSequenceItemList != null) {
                ...
                if (existingObjectiveLensNumericalAperture == 0) {
                    {
                        Attribute a = new DecimalStringAttribute(DicomDictionary.StandardDictionary.getTagFromName("ObjectiveLensNumericalAperture"));
                        a.addValue(objectiveLensNumericalAperture);
                        opticalPathSequenceItemList.put(a);
                    }
    ...
    private void addOpticalPathIdentifierAndDescriptionToOpticalPathSequence(AttributeList list, String opticalPathIdentifier, String opticalPathDescription) throws DicomException {
        if (opticalPathIdentifier != null && opticalPathIdentifier.length() > 0
            ...
            if (opticalPathSequenceItemList != null) {
                if (opticalPathIdentifier != null && opticalPathIdentifier.length() > 0) {
                    {
                        Attribute a = new ShortStringAttribute(DicomDictionary.StandardDictionary.getTagFromName("OpticalPathIdentifier"));
                        a.addValue(opticalPathIdentifier);
                        opticalPathSequenceItemList.put(a);
                    }
                ...
                if (opticalPathDescription != null && opticalPathDescription.length() > 0) {
                    {
                        Attribute a = new ShortTextAttribute(DicomDictionary.StandardDictionary.getTagFromName("OpticalPathDescription"));
                        a.addValue(opticalPathDescription);
                        opticalPathSequenceItemList.put(a);
                    }
    ...
    private AttributeList insertLossyImageCompressionHistory(AttributeList list,
...

        {
...

            {
                ...
                a.addValue(lossyImageCompression);
                list.put(a);
            }
    ...
    private AttributeList generateDICOMWholeSlideMicroscopyImageAttributes(AttributeList list,
        ...
        // Frame of Reference Module
        {
            ...
            a.addValue(frameOfReferenceUID);
            list.put(a);
        }    // (001244) (001256)
        ...
        boolean havePositionReference = !(xOffsetInSlideCoordinateSystem == 0 && yOffsetInSlideCoordinateSystem == 0);    // (001362)
        {
            ...
            a.addValue(havePositionReference ? "SLIDE_CORNER" : "UNKNOWN");
            list.put(a);
        }    // (001272)
        ...
        // Multi-frame Dimension Module - add it even though we are using TILED_FULL so not adding Per-Frame Functional Group :(
        {
            ...
            String dimensionOrganizationUID = uidGen.getAnotherNewUID();
            {
                ...
                list.put(saDimensionOrganizationSequence);
                {
                    ...
                    {
                        Attribute a = new UniqueIdentifierAttribute(TagFromName.DimensionOrganizationUID);
                        a.addValue(dimensionOrganizationUID);
                        itemList.put(a);
                    }
                }
            }
            {
                Attribute a = new CodeStringAttribute(TagFromName.DimensionOrganizationType);
                a.addValue("TILED_FULL");
                list.put(a);
            }
            {
                ...
                list.put(saDimensionIndexSequence);
                {
                    ...
                    {
                        AttributeTagAttribute a = new AttributeTagAttribute(TagFromName.DimensionIndexPointer);
                        a.addValue(TagFromName.RowPositionInTotalImagePixelMatrix);
                        itemList.put(a);
                    }
                    {
                        AttributeTagAttribute a = new AttributeTagAttribute(TagFromName.FunctionalGroupPointer);
                        a.addValue(TagFromName.PlanePositionSlideSequence);
                        itemList.put(a);
                    }
                    {
                        Attribute a = new UniqueIdentifierAttribute(TagFromName.DimensionOrganizationUID);
                    ...
                    {
                        Attribute a = new LongStringAttribute(TagFromName.DimensionDescriptionLabel);
                        a.addValue("Row Position");
                        itemList.put(a);
                    }
                }
                {
                    ...
                    {
                        AttributeTagAttribute a = new AttributeTagAttribute(TagFromName.DimensionIndexPointer);
                        a.addValue(TagFromName.ColumnPositionInTotalImagePixelMatrix);
                        itemList.put(a);
                    }
                    {
                        AttributeTagAttribute a = new AttributeTagAttribute(TagFromName.FunctionalGroupPointer);
        ...
        {
            Attribute a = new LongStringAttribute(DicomDictionary.StandardDictionary.getTagFromName("ContainerIdentifier"));
            a.addValue(containerIdentifier);
            list.put(a);
        }                    // Dummy value - should be able to override this :(
        {
            Attribute a = new SequenceAttribute(DicomDictionary.StandardDictionary.getTagFromName("IssuerOfTheContainerIdentifierSequence"));
            list.put(a);
        }
        CodedSequenceItem.putSingleCodedSequenceItem(list, DicomDictionary.StandardDictionary.getTagFromName("ContainerTypeCodeSequence"), "433466003", "SCT", "Microscope slide");    // No way of determining this :(
        {
            ...
            list.put(aSpecimenDescriptionSequence);
            {
                ...
                {
                    Attribute a = new LongStringAttribute(DicomDictionary.StandardDictionary.getTagFromName("SpecimenIdentifier"));
                    a.addValue(specimenIdentifier);
                    itemList.put(a);
                }    // Dummy value - should be able to override this :(
                {
                    Attribute a = new SequenceAttribute(DicomDictionary.StandardDictionary.getTagFromName("IssuerOfTheSpecimenIdentifierSequence"));
                    itemList.put(a);
                }
                {
                    Attribute a = new UniqueIdentifierAttribute(TagFromName.SpecimenUID);
                    a.addValue(specimenUID);
                    itemList.put(a);
                }
                {
                    Attribute a = new SequenceAttribute(DicomDictionary.StandardDictionary.getTagFromName("SpecimenPreparationSequence"));
                    itemList.put(a);
                }                        // Would be nice to be able to populate this :(
            }
        }

        // Multi-Resolution Pyramid Module CP 2135
        if (pyramidUID != null && pyramidUID.length() > 0) {
            Attribute a = new UniqueIdentifierAttribute(TagFromName.PyramidUID);
            a.addValue(pyramidUID);
            list.put(a);
        }
        // PyramidLabel
        // PyramidDescription
        ...
        if (acquisitionUID != null && acquisitionUID.length() > 0) {    // (001361)
            Attribute a = new UniqueIdentifierAttribute(TagFromName.AcquisitionUID);
            a.addValue(acquisitionUID);
            list.put(a);
        }
...

        {
            ...
            a.addValue("DERIVED");
            a.addValue("PRIMARY");
            a.addValue(imageFlavor);
            a.addValue(imageDerivation);
            list.put(a);
        }    // override default from CommonConvertedAttributeGeneration; same as FrameType
        ...
        if (!isLabel) {    // CP 2086 - actually condition is "is VOLUME", but we want it for THUMBNAIL too, except that since we know mmPerPixel for SVS overview images, we should include it
            Attribute a = new FloatSingleAttribute(DicomDictionary.StandardDictionary.getTagFromName("ImagedVolumeWidth"));
            a.addValue(imageWidth * mmPerPixelX);
            list.put(a);
        }
        if (!isLabel) {    // CP 2086 - actually condition is "is VOLUME", but we want it for THUMBNAIL too, except that since we know mmPerPixel for SVS overview images, we should include it
            Attribute a = new FloatSingleAttribute(DicomDictionary.StandardDictionary.getTagFromName("ImagedVolumeHeight"));
            a.addValue(imageLength * mmPerPixelY);
            list.put(a);
        }
        if (!isLabelOrOverview) {    // CP 2086 - actually condition is "is VOLUME", but we want it for THUMBNAIL too
            Attribute a = new FloatSingleAttribute(DicomDictionary.StandardDictionary.getTagFromName("ImagedVolumeDepth"));
            a.addValue(sliceThickness * 1000);
            list.put(a);    // (001315),(001317) Is in microns, not mm
        }

        {
            Attribute a = new UnsignedLongAttribute(DicomDictionary.StandardDictionary.getTagFromName("TotalPixelMatrixColumns"));
            a.addValue(imageWidth);
            list.put(a);
        }
        {
            Attribute a = new UnsignedLongAttribute(DicomDictionary.StandardDictionary.getTagFromName("TotalPixelMatrixRows"));
            a.addValue(imageLength);
            list.put(a);
        }
        {
            Attribute a = new UnsignedLongAttribute(DicomDictionary.StandardDictionary.getTagFromName("TotalPixelMatrixFocalPlanes"));
            a.addValue(1);
            list.put(a);
        }
        ...
        {
            Attribute a = new DecimalStringAttribute(TagFromName.ImageOrientationSlide);
            a.addValue(0.0);
            a.addValue(-1.0);
            a.addValue(0.0);
            a.addValue(-1.0);
        ...
        {
            Attribute a = new CodeStringAttribute(TagFromName.VolumetricProperties);
            a.addValue("VOLUME");
            list.put(a);
        }
        {
            Attribute a = new CodeStringAttribute(DicomDictionary.StandardDictionary.getTagFromName("SpecimenLabelInImage"));
            a.addValue("LABEL".equals(imageFlavor) || "OVERVIEW".equals(imageFlavor) ? "YES" : "NO");
            list.put(a);
        }        // Otherwise no way of determining this and most not :( (001328)
        {
            Attribute a = new CodeStringAttribute(DicomDictionary.StandardDictionary.getTagFromName("FocusMethod"));
            a.addValue("AUTO");
            list.put(a);
        }            // No way of determining this and most are :(
        {
            Attribute a = new CodeStringAttribute(DicomDictionary.StandardDictionary.getTagFromName("ExtendedDepthOfField"));
            a.addValue("NO");
            list.put(a);
        }        // No way of determining this and most not :(
        ...
        {
            Attribute a = new UnsignedShortAttribute(DicomDictionary.StandardDictionary.getTagFromName("RecommendedAbsentPixelCIELabValue"));
            a.addValue(0xFFFF);
            a.addValue(0);
            a.addValue(0);
            list.put(a);
        }        // white (0xFFFF is 100 per PS3.3 C.10.7.1.1)

        // Optical Path Module

        {
            Attribute a = new UnsignedLongAttribute(DicomDictionary.StandardDictionary.getTagFromName("NumberOfOpticalPaths"));
            a.addValue(1);
            list.put(a);
        }
        {
        ...
        if ("LABEL".equals(imageFlavor)) {
            ...
            if (list.get(DicomDictionary.StandardDictionary.getTagFromName("BarcodeValue")) == null) {
                Attribute a = new LongTextAttribute(DicomDictionary.StandardDictionary.getTagFromName("BarcodeValue"));
                list.put(a);
            }
            if (list.get(DicomDictionary.StandardDictionary.getTagFromName("LabelText")) == null) {
                Attribute a = new UnlimitedTextAttribute(DicomDictionary.StandardDictionary.getTagFromName("LabelText"));
                list.put(a);
            }
        }
    ...
    private int generateDICOMPyramidPixelDataModule(AttributeList list, String outputformat, String transferSyntax) throws DicomException, IOException {
        ...
        {
            Attribute a = new LongStringAttribute(pixelmedPrivatePyramidDataBlockReservation);
            a.addValue(pixelmedPrivateCreatorForPyramidData);
            list.put(a);
        }
        ...
        while (true) {
            ...
            if (!isFirstList) {
                ...
                a.addValue(transferSyntax);
                oldList.put(a);    // need this or won't decompress
            }
    ...
    private void parseTIFFImageDescription(String[] description, AttributeList descriptionList) throws DicomException {
        ...
        if (description != null && description.length > 0) {
            ...
            if (derivationDescription.length() > 0) {
                ...
                {
                    Attribute a = new ShortTextAttribute(TagFromName.DerivationDescription);
                    a.addValue(derivationDescription);
                    descriptionList.put(a);
                }
            }
        ...
        if (specimenPreparationStepContentItemSequence != null) {
            ...
            if (specimenPreparationSequence != null) {
                ...
                SequenceAttribute useSpecimenPreparationStepContentItemSequence = new SequenceAttribute(DicomDictionary.StandardDictionary.getTagFromName("SpecimenPreparationStepContentItemSequence"));
                {
                    ...
                    {
                        Attribute a = new UnlimitedTextAttribute(TagFromName.TextValue);
                        a.addValue(useSpecimenIdentifier);
                        itemList.put(a);
                    }
    ...
    private void convertTIFFPixelsToDicomMultiFrame(String jsonfile, UIDMap uidMap, TIFFFile inputFile, int dirNum, String outputFileName, int instanceNumber,
        ...
        if (includeCopyOfImageDescription && imageDescription != null && imageDescription.length() > 0) {    // (001347)
            if (imageDescription.length() <= 10240) {    // (001281)
                {
                    Attribute a = new LongTextAttribute(TagFromName.ImageComments);
                    a.addValue(imageDescription);
                    list.put(a);
                }
            } else {
                {
    ...
    private class WSIFrameOfReference {
        ...
        WSIFrameOfReference(ArrayList<TIFFImageFileDirectory> ifdlist, String[][] imageFlavorAndDerivationByIFD, double spacingrowmm, double spacingcolmm, double thicknessmm) throws DicomException {
            uidPyramid = uidGen.getAnotherNewUID();
                                    ...
                                    if (dimensionOrder != null) {
                                        try {
                                            if (dimensionOrder.length() == 5 && dimensionOrder.startsWith("XY")) {
                                                ...
                                                switch (minorOrder) {
                                                    case 'C':
                                                        minorOrderArray = channelForIFD;
                                                        minorCount = numberOfChannels;
                                                        break;
                                                    case 'Z':
                                                        minorOrderArray = zSectionForIFD;
                                                        minorCount = numberOfZSections;
                                                        break;
                                                    case 'T':
                                                        minorOrderArray = timepointForIFD;
                                                        minorCount = numberOfTimepoints;
                                                        break;
                                                    default:
                                                ...
                                                switch (middleOrder) {
                                                    case 'C':
                                                        middleOrderArray = channelForIFD;
                                                        middleCount = numberOfChannels;
                                                        break;
                                                    case 'Z':
                                                        middleOrderArray = zSectionForIFD;
                                                        middleCount = numberOfZSections;
                                                        break;
                                                    case 'T':
                                                        middleOrderArray = timepointForIFD;
                                                        middleCount = numberOfTimepoints;
                                                        break;
                                                    default:
                                                ...
                                                switch (majorOrder) {
                                                    case 'C':
                                                        majorOrderArray = channelForIFD;
                                                        majorCount = numberOfChannels;
                                                        break;
                                                    case 'Z':
                                                        majorOrderArray = zSectionForIFD;
                                                        majorCount = numberOfZSections;
                                                        break;
                                                    case 'T':
                                                        majorOrderArray = timepointForIFD;
                                                        majorCount = numberOfTimepoints;
                                                        break;
                                                    default:
            ...
            if (haveOverviewPixelSpacing
                ...
                uidOverview = uidGen.getAnotherNewUID();        // probably is no OVERVIEW (in 3D Histech) but just in case
                ...
                uidOverview = uidGen.getAnotherNewUID();
    ...
    private String[] addCommonDateTimeInformation(ArrayList<TIFFImageFileDirectory> ifdlist, AttributeList descriptionList) throws DicomException {
        ...
        }
        {
            ...
            {
                Attribute a = new DateTimeAttribute(TagFromName.AcquisitionDateTime);
                a.addValue(useCommonDate + useCommonTime);
                descriptionList.put(a);
            }
        }
    ...
    public TIFFToDicom(String jsonfile, String inputFileName, String outputFilePrefix, String outputFileSuffix, String modality, String sopClass, String transferSyntax, boolean addTIFF, boolean useBigTIFF, boolean alwaysWSI, boolean addPyramid, boolean mergeStrips, boolean autoRecognize, String channelFileName, boolean includeFileName, double spacingrowmm, double spacingcolmm, double thicknessmm, boolean includeCopyOfImageDescription, String uidFileName)
        ...
        if (isWSI) {
            pyramidUID = uidGen.getAnotherNewUID();
            acquisitionUID = uidGen.getAnotherNewUID();
        }
        ...
        AttributeList descriptionList = new AttributeList();        // keep track of stuff defined once but reusable for subsequent images
        {
            ...
            a.addValue(uidGen.getAnotherNewUID());
            descriptionList.put(a);
        }    // (001163)
        {
        ...
        String specimenUID = uidGen.getAnotherNewUID();
        ...
        for (TIFFImageFileDirectory ifd : ifdlist) {
            ...
            if (includeFileName) {    // (001314)
                {
                    Attribute a = new LongStringAttribute(pixelmedPrivateOriginalFileNameDataBlockReservation);
                    a.addValue(pixelmedPrivateCreator);
                    descriptionList.put(a);
                }
                {
                    Attribute a = new UnlimitedTextAttribute(pixelmedPrivateOriginalFileName);
                    a.addValue(inputFileName);
                    descriptionList.put(a);
                }
                ...
                {
                    Attribute a = new UnsignedShortAttribute(pixelmedPrivateOriginalTIFFIFDIndex);
                    a.addValue(dirNum);
                    descriptionList.put(a);
                }
            }
            ...
            while (keepLooking && endOptionsPosition > numberOfFixedArguments) {
                ...
                switch (option) {
                    ...
                    case "INCLUDEIMAGEDESCRIPTION":
                        includeCopyOfImageDescription = true;
                        --endOptionsPosition;
                        break;    // (001347)
                    case "DONOTINCLUDEIMAGEDESCRIPTION":
                        includeCopyOfImageDescription = false;
                        --endOptionsPosition;
                        break;

                    default:
                        if (arg[endOptionsPosition - 2].trim().toUpperCase().equals("CHANNELFILE")) {    // (001313)

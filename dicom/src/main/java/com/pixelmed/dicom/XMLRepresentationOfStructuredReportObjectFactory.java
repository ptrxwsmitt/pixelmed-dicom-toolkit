 ...
 * DICOM attributes, in the manner of {@link XMLRepresentationOfDicomObjectFactory XMLRepresentationOfDicomObjectFactory}.</p>
 *
 ...
 * @see XMLRepresentationOfDicomObjectFactory
...
public class XMLRepresentationOfStructuredReportObjectFactory {
	private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/dicom/XMLRepresentationOfStructuredReportObjectFactory.java,v 1.35 2022/01/21 19:51:19 dclunie Exp $";
	...
	private void addCodedConceptAttributesToDocumentNode(Node documentNode, Document document, CodedSequenceItem codedConcept) {
	 ...
	 */
	private void addContentItemsFromTreeToNode(ContentItem contentItem, Document document, Node documentParent, String id) {
		if (contentItem != null) {
			...
			if (documentNode != null) {
				...
				if (conceptName != null) {
					Node conceptNameNode = document.createElement("concept");
				...
				if (contentItem instanceof ContentItemFactory.ContainerContentItem) {
					...
					if (((ContentItemFactory.NumericContentItem)contentItem).hasFloatingPointValue()) {
						...
						if (floatingPointValueAsString != null && floatingPointValueAsString.length() > 0) {
							Node valueNode = document.createElement("double");
							documentNode.appendChild(valueNode);
					...
					if (((ContentItemFactory.NumericContentItem)contentItem).hasRationalValue()) {
						...
						if (numerator != null && numerator.length() > 0) {
							Node valueNode = document.createElement("numerator");
							documentNode.appendChild(valueNode);
						...
						if (denominator != null && denominator.length() > 0) {
							Node valueNode = document.createElement("denominator");
							documentNode.appendChild(valueNode);
					...
					if (unitsCode != null) {
						Node unitsCodeNode = document.createElement("units");
					...
					if (qualifier != null) {
						Node qualifierNode = document.createElement("qualifier");
					...
					if (graphicType != null) {	// regardless of whether zero length or not, need node to append data to
						Node graphicTypeNode = document.createElement(graphicType.toLowerCase(Locale.US));		// Locale explicitly specified, otherwise will fail to give the desired result if the default Locale is Turkish ("tr" or "tr_TR"); see "http://docs.oracle.com/javase/6/docs/api/java/lang/String.html#toLowerCase%28%29"
						...
						if (graphicData != null) {
							for (int i=0; i<graphicData.length; ++i) {
								Node coordinateNode = document.createElement(i%2 == 0 ? "x" : "y");
					...
					if (graphicType != null) {	// regardless of whether zero length or not, need node to append data to
						...
						if (graphicData != null) {
							for (int i=0; i<graphicData.length; ++i) {
								int xyz = i%3;
								Node coordinateNode = document.createElement(xyz == 0 ? "x" : (xyz == 1 ? "y" : "z"));
								graphicTypeNode.appendChild(coordinateNode);
								coordinateNode.appendChild(document.createTextNode(FloatFormatter.toStringOfFixedMaximumLength(graphicData[i],16/* maximum size of DS */,false/* non-numbers already handled */,Locale.US)));
							}
						}
					...
					if (referencedFrameOfReferenceUID != null && referencedFrameOfReferenceUID.length() > 0) {
						Node referencedFrameOfReferenceNode = document.createElement("frameofreference");
					...
					if (temporalRangeType != null) {	// regardless of whether zero length or not, need node to append data to
						Node temporalRangeTypeNode = document.createElement(temporalRangeType.toLowerCase(Locale.US));		// Locale explicitly specified, otherwise will fail to give the desired result if the default Locale is Turkish ("tr" or "tr_TR"); see "http://docs.oracle.com/javase/6/docs/api/java/lang/String.html#toLowerCase%28%29"
						documentNode.appendChild(temporalRangeTypeNode);
						{
							...
							if (referencedSamplePositions != null) {
								Node referencedSamplePositionsNode = document.createElement("samplepositions");
								temporalRangeTypeNode.appendChild(referencedSamplePositionsNode);
								for (int i=0; i<referencedSamplePositions.length; ++i) {
									Node referencedSamplePositionNode = document.createElement("position");
						...
						}
						{
							...
							if (referencedTimeOffsets != null) {
								Node referencedTimeOffsetsNode = document.createElement("timeoffsets");
								temporalRangeTypeNode.appendChild(referencedTimeOffsetsNode);
								for (int i=0; i<referencedTimeOffsets.length; ++i) {
									Node referencedTimeOffsetNode = document.createElement("offset");
						...
						}
						{
							...
							if (referencedDateTimes != null) {
								Node referencedDateTimesNode = document.createElement("datetimes");
								temporalRangeTypeNode.appendChild(referencedDateTimesNode);
								for (int i=0; i<referencedDateTimes.length; ++i) {
									Node referencedDateTimeNode = document.createElement("datetime");
				...
				else if (contentItem instanceof ContentItemFactory.CompositeContentItem) {
					{
						...
						if (referencedSOPClassUID != null && referencedSOPClassUID.length() > 0) {
							Node referencedSOPClassUIDNode = document.createElement("class");
					...
					}
					{
						...
						if (referencedSOPInstanceUID != null && referencedSOPInstanceUID.length() > 0) {
							Node referencedSOPInstanceUIDNode = document.createElement("instance");
					...
					if (contentItem instanceof ContentItemFactory.ImageContentItem) {
						ContentItemFactory.ImageContentItem imageContentItem = (ContentItemFactory.ImageContentItem)contentItem;
						{
							...
							if (referencedFrameNumber != 0) {
								Node referencedFrameNumberNode = document.createElement("frame");
						...
						}
						{
							...
							if (referencedSegmentNumber != 0) {
								Node referencedSegmentNumberNode = document.createElement("segment");
						...
						}
						{
							...
							if (presentationStateSOPClassUID != null && presentationStateSOPClassUID.length() > 0
							 || presentationStateSOPInstanceUID != null && presentationStateSOPInstanceUID.length() > 0) {
								Node presentationStateReference = document.createElement("presentationstate");
								documentNode.appendChild(presentationStateReference);
								
								Node presentationStateSOPClassUIDNode = document.createElement("class");
								...
								Node presentationStateSOPInstanceUIDNode = document.createElement("instance");
						...
						}
						{
							...
							if (realWorldValueMappingSOPClassUID != null && realWorldValueMappingSOPClassUID.length() > 0
							 || realWorldValueMappingSOPInstanceUID != null && realWorldValueMappingSOPInstanceUID.length() > 0) {
								Node realWorldValueMappingReference = document.createElement("realworldvaluemapping");
								documentNode.appendChild(realWorldValueMappingReference);
								
								Node realWorldValueMappingSOPClassUIDNode = document.createElement("class");
								...
								Node realWorldValueMappingSOPInstanceUIDNode = document.createElement("instance");
						...
						if (referencedWaveformChannels != null && referencedWaveformChannels.length > 0) {
							Node referencedWaveformChannelsNode = document.createElement("channels");
							documentNode.appendChild(referencedWaveformChannelsNode);
							for (int i=0; i<referencedWaveformChannels.length; ++i) {
								Node channelNode = document.createElement("channel");
	...
	public Document getDocument(StructuredReport sr,AttributeList list) {
		...
		Node rootElement = document.createElement("DicomStructuredReport");
		...
		if (list != null) {
			...
			Node headerElement = document.createElement("DicomStructuredReportHeader");
		...
		if (sr != null) {
			Node contentElement = document.createElement("DicomStructuredReportContent");
...
//System.err.println("XMLRepresentationOfStructuredReportObjectFactory.getNextContentItemFromXMLNodeSiblings(): relationship with child = "+childRelationshipType);
	...
	public StructuredReport getStructuredReport(Document document) throws DicomException, ParserConfigurationException {
		...
		Node dicomStructuredReportElement = document.getDocumentElement();	// should be DicomStructuredReport
...
//System.err.println("XMLRepresentationOfStructuredReportObjectFactory.getAttributeList(Document document): List is "+list);
	 ...
	 * @param	indent
	 */
	public static String toString(Node documentNode, int indent) {
		...
		str.append("\n");
		++indent;
		for (Node child = documentNode.getFirstChild(); child != null; child = child.getNextSibling()) {

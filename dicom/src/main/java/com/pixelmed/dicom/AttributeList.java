...
/**
 * <p>The {@link AttributeList AttributeList} class maintains a list of individual DICOM attributes.</p>
 *
 ...
 *
 * <p>The class is actually implemented by extending {@link TreeMap java.util.TreeMap}
 * as a map of {@link AttributeTag AttributeTag} keys to
 * {@link Attribute Attribute} values. Consequently, all the methods
 ...
 * order of the {@link AttributeTag AttributeTag} keys, since
 * that is how {@link AttributeTag AttributeTag} implements
 * {@link Comparable Comparable}.</p>
 *
 ...
 *
 *
 * <p>Note also that the {@link Attribute Attribute} class provides some useful
 ...
 * {@link SpecificCharacterSet#SpecificCharacterSet(String[]) SpecificCharacterSet()}
 ...
 * @see Attribute
 * @see AttributeTag
 * @see FileMetaInformation
 * @see SpecificCharacterSet
 * @see TagFromName
 * @see TransferSyntax
 *
 * @author	dclunie
...
public class AttributeList extends TreeMap<AttributeTag,Attribute> {
	private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/dicom/AttributeList.java,v 1.210 2022/01/21 19:51:14 dclunie Exp $";
					...
					if (decompressPixelData) {
						...
						if (frames <= maximumInMemoryNumberOfCompressedFrames
						 || !ei.framesAreSeparated()		// have not yet implemented leaving all frames in one compressed fragment left on disk :( (001037)
							...
							slf4jlogger.debug("read(): leave all compressed frames on disk");
...
//System.err.println("Writing "+a);
	 ...
	 * does indicate that the map contains no mapping for the key, unlike {@link TreeMap#get(Object) java.util.get(Object)}
	 ...
	 *
	 * @see SOPClass#isStructuredReport(String)
	 *
	 ...
	 *
	 * @see DicomDictionary#StandardDictionary
	 *
	 ...
	 * <p>Does nothing if the attribute was not already present.</p>
	 *
	 * @deprecated	See {@link AttributeList#replaceWithValueIfPresent(AttributeTag,String) replaceWithValueIfPresent()}
	 *
	 * @param	tag			key (tag) for which the attribute should be replaced
...
//System.err.println("AttributeList.getPrivateCreatorString(): creator is "+creator);
	 ...
	 *
	 * <p>Overlay attributes are those for which {@link AttributeTag#isOverlayGroup() com.pixelmed.dicom.AttributeTag.isOverlayGroup()} returns true.</p>
	 *
	 ...
	 *
	 * <p>Curve attributes are those for which {@link AttributeTag#isCurveGroup() com.pixelmed.dicom.AttributeTag.isCurveGroup()} returns true.</p>
	 */
	 ...
	 * for {@link Attribute#toString() com.pixelmed.dicom.Attribute.toString()}.</p>
	 *
	 * @return			the string
	 */
	 ...
	 *
	 * <p>Each attribute is written to a separate line, in the form defined
	 * for {@link Attribute#toString(DicomDictionary dictionary) com.pixelmed.dicom.Attribute.toString(DicomDictionary dictionary)}.</p>
	 *
	 ...
	 * @return				the values as a delimited {@link String String}
	 */
...
//System.err.println("AttributeList.setFileUsedByOnDiskAttributes(): setting OtherWordAttributeOnDisk to file = "+file);
	 ...
	 *
	 * @param	t						key ({@link AttributeTag AttributeTag} tag) with which the generated attribute is to be associated
	 * @param	specificCharacterSet	the {@link SpecificCharacterSet SpecificCharacterSet} to be used text values
...
//System.err.println("AttributeList.insertSuitableSpecificCharacterSetForAllStringValues(): specificCharacterSetValue = "+specificCharacterSetValue);
	...
	// compare this to CompressedFrameDecoder.chooseReaderWantedBasedOnTransferSyntax()
...
//System.err.println("Undefined length encapsulated Pixel Data in RLE");
			...
			if (tsuid.equals(TransferSyntax.JPEGBaseline) || tsuid.equals(TransferSyntax.JPEGExtended) || tsuid.equals(TransferSyntax.JPEGFullProgressionNonHierarchical1012)) {
				readerWanted="JPEG";
...
//System.err.println("Undefined length encapsulated Pixel Data in JPEG-LS");
	...
	/**
	 * <p>Returns a reference to the {@link IIOMetadata IIOMetadata} object for the selected frame, or null if none was available during reading. </p>
	 *
	 * @param	frame	the frame number, from 0
	 * @return	an {@link IIOMetadata IIOMetadata} object, or null.
	 */
...
//System.err.println("findAllNestedReferencedSOPInstanceUIDs(): adding "+referencedSOPInstanceUID);
	...
	/**
	 * <p>Make a new {@link AttributeList AttributeList} from String keyword and value pairs.</p>
	 *
	 ...
	 * @return						a new {@link AttributeList AttributeList}
	...
	public static void main(String arg[]) {
		if (arg.length == 3 && arg[2].equals("TEST")) {
			...
			if (arg.length == 1) {
				...
				outputTransferSyntax = TransferSyntaxFromName.getUID(arg[2]);
				outputFileName = arg[3];
				outputMeta = ! arg[4].toUpperCase(Locale.US).contains("NO");
			}
			try {
				AttributeList list = new AttributeList();

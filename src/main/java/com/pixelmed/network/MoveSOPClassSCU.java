...
public class MoveSOPClassSCU extends SOPClass {
	...
	protected class CMoveResponseHandler extends CompositeResponseHandler {
		...
		protected void evaluateStatusAndSetSuccess(AttributeList list) {
			...
			if (slf4jlogger.isTraceEnabled()) slf4jlogger.trace("CMoveResponseHandler.evaluateStatusAndSetSuccess: status =0x{}",Integer.toHexString(status));
		...
		protected void makeUseOfDataSet(AttributeList list) {
			if (slf4jlogger.isTraceEnabled()) slf4jlogger.trace("CMoveResponseHandler.makeUseOfDataSet:\n{}",list.toString());
	 ...
	 * @param	affectedSOPClass	the SOP Class defining which retrieve model, e.g. {@link SOPClass#StudyRootQueryRetrieveInformationModelMove SOPClass.StudyRootQueryRetrieveInformationModelMove}
	 * @param	debugLevel			ignored
	 * @throws	IOException

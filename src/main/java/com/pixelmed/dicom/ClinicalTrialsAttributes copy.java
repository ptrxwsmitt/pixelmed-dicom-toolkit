...
abstract public class ClinicalTrialsAttributes {
	private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/dicom/ClinicalTrialsAttributes.java,v 1.117 2020/06/08 18:11:34 dclunie Exp $";
	...
	protected static Map<String,String> mapOfOriginalToReplacementUIDs = null;
	protected static Map<String,String> mapOfReplacementToOriginalUIDs = null;
...
//System.err.println("ClinicalTrialsAttributes.isSafePrivateSequenceAttribute(): checking "+tag+" creator "+creator);
	...
	public static void flushMapOfUIDs() {
		...
		mapOfReplacementToOriginalUIDs = null;
	}
		...
		if (forRemovalOrRemapping != null) {
			...
			while (i2.hasNext()) {
				...
				if (handleUIDs == HandleUIDs.remove) {
					...
					//if (tag.equals(dictionary.getTagFromName("SOPInstanceUID"))) slf4jlogger.info("removeOrRemapUIDAttributes(): requesting replacement of SOP Instance UID {}",originalUIDValue);
					if (originalUIDValue != null) {
						String replacementUIDValue = null;
						if (mapOfOriginalToReplacementUIDs == null) {
							mapOfOriginalToReplacementUIDs = new HashMap<String,String>();
						}
						if (mapOfReplacementToOriginalUIDs == null) {
							mapOfReplacementToOriginalUIDs = new HashMap<String,String>();
						}
						replacementUIDValue = mapOfOriginalToReplacementUIDs.get(originalUIDValue);
						if (replacementUIDValue == null) {
							if (uidGenerator == null) {
								uidGenerator = new UIDGenerator();
							}
							String newReplacementUIDValue = uidGenerator.getAnotherNewUID();
							if (tag.equals(dictionary.getTagFromName("SOPInstanceUID"))) slf4jlogger.info("removeOrRemapUIDAttributes(): newly generated ReplacementUIDValue {}",newReplacementUIDValue);
							String originalUIDValueForNewReplacementUIDValue = mapOfReplacementToOriginalUIDs.get(newReplacementUIDValue);	// should be null else should have been returned by mapOfOriginalToReplacementUIDs.get(originalUIDValue)
							while (originalUIDValueForNewReplacementUIDValue != null) {
								slf4jlogger.error("Duplicate UID was issued by new UIDGenerator() - iterating until unique :(");
								newReplacementUIDValue = uidGenerator.getAnotherNewUID();
								originalUIDValueForNewReplacementUIDValue = mapOfReplacementToOriginalUIDs.get(newReplacementUIDValue);
							}
							mapOfOriginalToReplacementUIDs.put(originalUIDValue,newReplacementUIDValue);
							mapOfReplacementToOriginalUIDs.put(newReplacementUIDValue,originalUIDValue);
							replacementUIDValue = newReplacementUIDValue;
						}
						else {
							if (tag.equals(dictionary.getTagFromName("SOPInstanceUID"))) slf4jlogger.info("removeOrRemapUIDAttributes(): found for SOP Instance UID {} previous replacement {}",originalUIDValue,replacementUIDValue);
						}
						...
						list.put(tag,a);
						if (tag.equals(dictionary.getTagFromName("SOPInstanceUID"))) slf4jlogger.info("removeOrRemapUIDAttributes(): replacing SOP Instance UID {} with {}",originalUIDValue,replacementUIDValue);
					}
					else {

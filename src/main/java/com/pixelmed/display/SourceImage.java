...
public class SourceImage {
	 ...
	 *
	 * <p>Reading of the pixel data for a frame (and allocation of memory for it) is deferred until {@link SourceImage#getBufferedImage(int) SourceImage.getBufferedImage(int)} is called.</p>
	 *
	...
	/**
	 * <p>Set the rectanguler clipping region for the selected frame.</p>
	 *
	 * @param	i	frame number (from 0)
	 */
	public void setClipRect(int i,Rectangle clipRect) {
		if (clipRects == null) {
			slf4jlogger.trace("setClipRect(): lazy allocation of clipRects array length = {}",nframes);
			clipRects = new Rectangle[nframes];
		}
		slf4jlogger.trace("setClipRect(): setting clipRect for frame = {}",i);
		clipRects[i] = clipRect;
	}
	 ...
	 *
	 * @param	filename
	 * @param	applyICCProfileIfPresent	whether or not to apply any ICC profile present in the attribute list
	 * @throws	IOException
...
//System.err.println(list);
	 ...
	 * a file.</p>
	 *
	 * <p>Applies any ICC Profile that is present.</p>
	 *
	 * @param		filename
	 * @throws	IOException
	 * @throws	DicomException
	...
	public SourceImage(String filename) throws IOException, DicomException {
		this(filename,true/*applyICCProfileIfPresent*/);
	}
...
//System.err.println(list);
	...
	public SourceImage(DicomInputStream i) throws IOException, DicomException {
		this(i,true/*applyICCProfileIfPresent*/);
	}
	...
	public SourceImage(AttributeList list) throws DicomException {
		this(list,true/*applyICCProfileIfPresent*/);
	}
	...
	private abstract class ShortBufferedImageSource extends BufferedImageSource {
		...
		ShortBufferedImageSource(byte[][] compressedData,int bytesPerSample,int width,int height,int samples,String compressedDataTransferSyntaxUID,ColorSpace colorSpace,boolean codecConvertsYBRtoRGB) {
			...
			try {
				...
				decoder = new CompressedFrameDecoder(compressedDataTransferSyntaxUID,compressedData,bytesPerSample,width,height,samples,colorSpace,codecConvertsYBRtoRGB);
			}
		...
		ShortBufferedImageSource(ByteFrameSource compressedDataFrameSource,int bytesPerSample,int width,int height,int samples,String compressedDataTransferSyntaxUID,ColorSpace colorSpace,boolean codecConvertsYBRtoRGB) {
			super(width*height*samples);
			try {
				...
				decoder = new CompressedFrameDecoder(compressedDataTransferSyntaxUID,compressedDataFrameSource,bytesPerSample,width,height,samples,colorSpace,codecConvertsYBRtoRGB);
			}
			catch (DicomException e) {
				slf4jlogger.error("",e);
				decoder = null;
	...
	private abstract class ByteBufferedImageSource extends BufferedImageSource {
		...
		ByteBufferedImageSource(byte[][] compressedData,int width,int height,int samples,String compressedDataTransferSyntaxUID,ColorSpace colorSpace,boolean codecConvertsYBRtoRGB) {
			super(width*height*samples);
			try {
				slf4jlogger.debug("ByteBufferedImageSource(): creating CompressedFrameDecoder with bytesPerSample = 1");
				decoder = new CompressedFrameDecoder(compressedDataTransferSyntaxUID,compressedData,1/*bytesPerSample*/,width,height,samples,colorSpace,codecConvertsYBRtoRGB);
			}
			catch (DicomException e) {
				slf4jlogger.error("",e);
				decoder = null;
		...
		ByteBufferedImageSource(ByteFrameSource compressedDataFrameSource,int width,int height,int samples,String compressedDataTransferSyntaxUID,ColorSpace colorSpace,boolean codecConvertsYBRtoRGB) {
			super(width*height*samples);
			try {
				slf4jlogger.debug("ByteBufferedImageSource(): creating CompressedFrameDecoder with bytesPerSample = 1");
				decoder = new CompressedFrameDecoder(compressedDataTransferSyntaxUID,compressedDataFrameSource,1/*bytesPerSample*/,width,height,samples,colorSpace,codecConvertsYBRtoRGB);
			}
			catch (DicomException e) {
				slf4jlogger.error("",e);
				decoder = null;
	...
	private class SignedByteGrayscaleBufferedImageSource extends ByteBufferedImageSource {
		...
		SignedByteGrayscaleBufferedImageSource(byte[][] compressedData,int width,int height,int mask,int signbit,int extend,int largestGray,String compressedDataTransferSyntaxUID) {
			super(compressedData,width,height,1,compressedDataTransferSyntaxUID,ColorSpace.getInstance(ColorSpace.CS_GRAY),false/*codecConvertsYBRtoRGB*/);
			doCommonConstructorStuff(mask,signbit,extend,largestGray);
		}
		...
		SignedByteGrayscaleBufferedImageSource(ByteFrameSource compressedDataFrameSource,int width,int height,int mask,int signbit,int extend,int largestGray,String compressedDataTransferSyntaxUID) {
			super(compressedDataFrameSource,width,height,1,compressedDataTransferSyntaxUID,ColorSpace.getInstance(ColorSpace.CS_GRAY),false/*codecConvertsYBRtoRGB*/);
			doCommonConstructorStuff(mask,signbit,extend,largestGray);
		}
	...
	private class UnsignedByteGrayscaleBufferedImageSource extends ByteBufferedImageSource {
		...
		UnsignedByteGrayscaleBufferedImageSource(File[] filesPerFrame,long[] byteOffsets,int width,int height,int mask,int largestGray) {
			super(filesPerFrame,byteOffsets,width*height*1/*samples*/);
			doCommonConstructorStuff(mask,largestGray);
		}
	...
	private class SignedShortGrayscaleBufferedImageSource extends ShortBufferedImageSource {
		...
		SignedShortGrayscaleBufferedImageSource(byte[][] compressedData,int width,int height,int mask,int signbit,int extend,int largestGray,String compressedDataTransferSyntaxUID) {
			super(compressedData,2/*bytesPerSample*/,width,height,1,compressedDataTransferSyntaxUID,ColorSpace.getInstance(ColorSpace.CS_GRAY),false/*codecConvertsYBRtoRGB*/);
			doCommonConstructorStuff(mask,signbit,extend,largestGray);
		}
		...
		SignedShortGrayscaleBufferedImageSource(ByteFrameSource compressedDataFrameSource,int width,int height,int mask,int signbit,int extend,int largestGray,String compressedDataTransferSyntaxUID) {
			super(compressedDataFrameSource,2/*bytesPerSample*/,width,height,1,compressedDataTransferSyntaxUID,ColorSpace.getInstance(ColorSpace.CS_GRAY),false/*codecConvertsYBRtoRGB*/);
			doCommonConstructorStuff(mask,signbit,extend,largestGray);
		}
	...
	private class CompressedByteThreeComponentColorBufferedImageSource extends BufferedImageSource /* extending BufferedImageSource also leverages its last frame caching mechanism */ {
		...
		CompressedByteThreeComponentColorBufferedImageSource(byte[][] compressedData,int width,int height,ColorSpace colorSpace,String compressedDataTransferSyntaxUID,boolean codecConvertsYBRtoRGB) {
			...
			try {
				...
				decoder = new CompressedFrameDecoder(compressedDataTransferSyntaxUID,compressedData,1/*bytesPerSample*/,width,height,3/*samplesPerPixel*/,colorSpace,codecConvertsYBRtoRGB);
			}
			catch (DicomException e) {
				slf4jlogger.error("",e);
				decoder = null;
		...
		CompressedByteThreeComponentColorBufferedImageSource(ByteFrameSource compressedDataFrameSource,int width,int height,ColorSpace colorSpace,String compressedDataTransferSyntaxUID,boolean codecConvertsYBRtoRGB) {
			super(width*height*3/*samples*/);	// nframesamples is actually irrelevant ... BufferedImageSource does nothing with this information :(
			try {
				...
				decoder = new CompressedFrameDecoder(compressedDataTransferSyntaxUID,compressedDataFrameSource,1/*bytesPerSample*/,width,height,3/*samplesPerPixel*/,colorSpace,codecConvertsYBRtoRGB);
			}
			catch (DicomException e) {
				slf4jlogger.error("",e);
				decoder = null;
	...
	void constructSourceImage(AttributeList list) throws DicomException {
		...
		if (vPhotometricInterpretation.equals("MONOCHROME2")) {
			...
			if (list.getDecompressedPhotometricInterpretation(vPhotometricInterpretation).equals("RGB")) {
				codecConvertsYBRtoRGB = true;
			}
			else {
		...
		if (applyICCProfileIfPresent) {
			...
			if (aICCProfile != null) {
				...
				else if (iccProfileBytes.length == 0) {
					slf4jlogger.error("constructSourceImage(): ICC Profile present but zero length");
				}
				else {
					slf4jlogger.trace("constructSourceImage(): ICC Profile length = {}",iccProfileBytes.length);
					if (slf4jlogger.isTraceEnabled()) slf4jlogger.trace("constructSourceImage(): ICC Profile =\n{}\n",com.pixelmed.utils.HexDump.dump(iccProfileBytes));
					try {
			...
			if (bufferedImageSource == null) {	// did not attempt to memory map or attempt failed
				...
				if (a instanceof OtherByteAttributeOnDisk) {
					...
					if (allowDeferredReadFromFileIfNotMemoryMapped) {
						...
						File[] filesPerFrame = ((OtherByteAttributeMultipleFilesOnDisk)a).getFiles();
						long[] byteOffsets =  ((OtherByteAttributeMultipleFilesOnDisk)a).getByteOffsets();
						if (signed) {
							bufferedImageSource = new SignedByteGrayscaleBufferedImageSource(filesPerFrame,byteOffsets,width,height,mask,signbit,extend,largestGray);
						}
						else {
							bufferedImageSource = new UnsignedByteGrayscaleBufferedImageSource(filesPerFrame,byteOffsets,width,height,mask,largestGray);
						}
					}
					else {
						slf4jlogger.debug("constructSourceImage(): OB left on disk and deferred read disabled so reading into contiguous byte array");
						byte[] data = ((OtherByteAttributeMultipleFilesOnDisk)a).getByteValues();
						if (signed) {
...
long currentTime = System.currentTimeMillis();
slf4jlogger.info("constructSourceImage(): single bit read from OW = {} ms",(currentTime-startTime));
			...
			if (compressedData != null) {
				CompressedByteThreeComponentColorBufferedImageSource compressedSource = new CompressedByteThreeComponentColorBufferedImageSource(compressedData,width,height,srcColorSpace,compressedDataTransferSyntaxUID,codecConvertsYBRtoRGB);
				bufferedImageSource = compressedSource;
			}
			else if (compressedDataFrameSource != null) {
				CompressedByteThreeComponentColorBufferedImageSource compressedSource = new CompressedByteThreeComponentColorBufferedImageSource(compressedDataFrameSource,width,height,srcColorSpace,compressedDataTransferSyntaxUID,codecConvertsYBRtoRGB);
				bufferedImageSource = compressedSource;
			}
			else if (byplane) {
				if (isChrominanceHorizontallyDownsampledBy2) {
	...
	public BufferedImage getBufferedImage(int i,boolean useICC) {
		...
		if (img != null) {
			if (clipRects != null) {
				Rectangle clipRect = clipRects[i];
				if (clipRect != null) {
					img = BufferedImageUtilities.clipToRectangleWithGraphicsDraw(img,clipRect,Color.BLACK);		// ? should use value of "background" somehow
				}
			}
		}
		if (img != null) {
			// Why reset the min and max ? They may not previously have been set or frame not fetched from memory mapped file yet
			if (bufferedImageSource != null) {	// (001293)
